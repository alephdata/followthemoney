...
Generating graph data is a difficult process. The size of the datasets we want to process using followthemoney (FtM) makes it impossible to incrementally build nodes and egdes in memory like you would in [NetworkX](https://networkx.org/). Instead, we use a stream-based solution for constructing graph entities. That is why the toolkit supports *entity fragments* and *aggregation*.
...
When [turning this data into FtM](/guide/mappings), we’d create three entities for each row: a [](schema://Company), a [](schema://Person) and a [](schema://Directorship) that connects the two.
