...
When `turning this data into FtM <mappings>`, we\'d create three entities for each row: a `schema-Company`, a `schema-Person` and a `schema-Directorship` that connects the two.
