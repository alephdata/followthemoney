...
The core interfaces of _followthemoney_ are simple: each running instance of the library has a `Model` singleton, which holds a set of `Schema` definitions (e.g. `schema:Person`). Each schema defines a set of `properties` (e.g. `name`, `birthDate`) which give meaning to how values can be associated with entities of a given schema.
...
<LinkCard
  title="Schema class"
  link="/reference/python/followthemoney/schema.html#Schema"
/>

<LinkCard
  title="EntityProxy class"
  link="/reference/python/followthemoney/proxy.html#EntityProxy"
/>
