...
def _normalize_names(names: Iterable[str]) -> Generator[str, None, None]:
    ...
    for name in names:
        ...
        if fp is not None and len(fp) > 3 and fp not in seen:
            seen.add(fp)
            yield fp
