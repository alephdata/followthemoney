...
2. Sometimes we want to talk about the inverse of one of these edges. That's
   ...
   :ref:`schema-Person` schema. This is mostly used to store, for example,
   the labels that should be used to talk about the passports linked to a
   person. But it's a :attr:`~followthemoney.property.Property.stub`
...
3. The weakness of this model is that edges don't have properties, whereas
   ...
   are limited in time (hence :ref:`schema-Interval`). So we added a hacky work-around that
